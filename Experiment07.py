...
pcg_iters_pmr = np.load("data/Experiment07_" + matrix + "_pcg_iters_pmr_spai.npz")
pcg_iters_pcg = np.load("data/Experiment07_" + matrix + "_pcg_iters_pcg_spai.npz")
pcg_iters_lopmr = np.load("data/Experiment07_" + matrix + "_pcg_iters_lopmr_spai.npz")
...
ax.plot(np.arange(0, iters), pcg_iters_pmr, color='k', linestyle=(0, (3, 1, 1, 1, 1, 1)), label='PCG (PMR-SPAI)', linewidth=2)
ax.plot(np.arange(0, iters), pcg_iters_pcg, 'k:', label='PCG (PCG-SPAI)', linewidth=2)
ax.plot(np.arange(0, iters), pcg_iters_lopmr, 'k-', label='PCG (LOPMR-SPAI)', linewidth=2)
