...
using Pkg
Pkg.activate(".")
using SparseApproximateInverses: pmr_spd, pcg, lopmr_spd
...
if matrix == "bundle1" # Computer graphics problem
  ...
  itmax = Dict("pmr"=>200, "pcg"=>200, "lopmr"=>200)
...
elseif matrix == "wathen100" # Random 2D/3D problem
  ...
  itmax = Dict("pmr"=>100, "pcg"=>100, "lopmr"=>100)
...
npzwrite("data/Experiment03_" * matrix * "_metadata_pmr_spd.npz", 
         [val_LR, val_SR])
...
println("val_SR = $val_SR")
npzwrite("data/Experiment03_" * matrix * "_metadata_lopmr_spd.npz", 
         [val_LR, val_SR])